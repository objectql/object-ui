...
# needed adapting. The only edits below the header are the two KNOWN HOLE sections, whose
# issue references are re-pointed at this repo's own cards for the same two defects.
...
echo "== KNOWN HOLE #7260: NotebookEdit's path key is notebook_path, which this hook never reads =="
...
# When #7260 is fixed, these become block / allow / allow by the notebook's own path.
# (Same defect as objectstack-ai/objectstack#11810 — the hooks share these lines.)
