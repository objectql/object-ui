...
/**
 * objectui#4528 structural guard — the `packages/plugin-dashboard` sibling of
 ...
 * offender arriving — which is exactly how `DashboardRenderer` kept the defect
 * for the whole life of #4438. objectui#4528 direction 3 (one guard over every
 ...
 *     reads (`schema` included) is `any`; and
 ...
 *     `keyof React.ComponentProps< typeof DashboardRenderer >` was
 *     `string | number` and `...['onWidgetClick']` was `any`, while
 *     `DashboardRendererProps` went on declaring
 *     `(widgetId: string | null) => void`.
 *
 ...
 * DESTRUCTURES a `schema` prop. That heuristic misses this package's public
 * component: `DashboardRenderer` takes `(props, ref)` whole and forwards it, so
 * it destructures nothing — yet it is precisely the call-site half objectui#4528
 * measured. So this guard's population is instead "every `forwardRef` whose
 * props TYPE ARGUMENT is a type declared in the SAME file", i.e. every site
 * where this package owns the props contract and a source scan can actually
 * read it. A props type imported from elsewhere stays out of reach of a source
 * scan and is not claimed to be covered (`ListViewBlock` in the sibling package
 * is the worked example) — it is covered at its declaration site instead.
 *
 * ## If this fails
 *
...
// packages/plugin-dashboard/src/__tests__  ->  packages/plugin-dashboard/src
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isCallExpression(node)) {
      ...
      if (isForwardRef) {
        ...
        // elsewhere is out of a source scan's reach and is covered where it is
        // declared instead.
        let ownsProps = false;
        if (typeArg) {
          ownsProps =
...
describe('objectui#4528 — plugin-dashboard forwardRef props must not be erased', () => {
  it('finds the population (guards against a broken scan)', () => {
    // If this collapses, the walk or the AST matcher has gone stale and the
    // guard would silently pass on nothing. Two sites exist at the time of
    // writing — `DashboardRendererInner` and the public `DashboardRenderer`,
    // both on `DashboardRendererProps`. The floor is deliberately loose so
    // adding or removing one component does not fail the wrong assertion.
    expect(ALL_SITES.length).toBeGreaterThanOrEqual(2);
  });
  ...
  it('detects the shapes it is meant to ban (guards against a dead matcher)', () => {
    ...
    //    prop of `DashboardRendererProps`.
    const named = scan(
      'interface P { schema: XSchema; onWidgetClick?: (id: string | null) => void; [key: string]: any }\n' +
        'const C = forwardRef<HTMLDivElement, P>(({ schema, ...props }, ref) => null);',
    );
    expect(named).toHaveLength(1);
    expect(named[0].indexSignatureOnTypeArg).toBe(true);
    expect(named[0].annotated).toBe(false);

    // 2. The PUBLIC half, which the objectui#4422 guard's `schema`-destructuring
    //    heuristic cannot see: props taken whole and forwarded. `DashboardRenderer`
    //    itself is spelled exactly this way, so a guard blind to it would have
    //    passed on the very component this card is about.
    const whole = scan(
      'interface P { schema: XSchema; [key: string]: any }\n' +
        'const C = forwardRef<HTMLDivElement, P>((props, ref) => null);',
    );
    ...
    const fixed = scan(
      'interface P { schema: XSchema; className?: string }\n' +
        'const C = forwardRef<HTMLDivElement, P>(' +
