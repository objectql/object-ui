...
const zh = {
  ...
  console: {
    ...
    datasource: {
      title: '数据源',
      subtitle: '连接外部数据库,并将其数据表同步为对象。',
      refresh: '刷新',
      new: '新建数据源',
      loading: '加载中…',
      emptyPrefix: '还没有数据源。点击 ',
      emptyAction: '新建数据源',
      emptySuffix: ' 来连接一个。',
      confirmDelete: '删除数据源“{{name}}”?此操作无法撤销。',
      row: {
        test: '测试',
        sync: '同步对象',
        edit: '编辑',
      ...
      editor: {
        editTitle: '编辑“{{name}}”',
        newTitle: '新建数据源',
        editDesc: '更新连接。凭据留空则保留当前凭据。',
        newDesc: '连接一个外部数据库。凭据会被加密,绝不存入元数据。',
        name: '名称',
        namePlaceholder: 'my_database',
        nameInvalid: '仅限小写字母、数字和下划线;不能以数字开头。',
        label: '标签',
        labelPlaceholder: '便于识别的名称',
        driver: '驱动',
        driverPlaceholder: '选择驱动',
        schemaMode: '模式(Schema)处理方式',
        // 按 SCHEMA_MODES 的 value 取键。
        schemaModes: {
          managed: '托管(由 ObjectStack 管理 schema)',
          external: '外部(按现状读取已有 schema)',
          'validate-only': '仅校验',
        },
        secretKeepPlaceholder: '••••••••(留空则保留)',
        optionSelect: '请选择…',
        testConnection: '测试连接',
        testOk: '连接正常',
        testOkMs: '连接正常({{ms}}ms)',
      ...
      sync: {
        title: '从“{{name}}”同步对象',
        desc: '选择要导入为对象的远端数据表。每张表将生成一个对象定义。',
        introspecting: '正在读取表结构…',
        noTables: '此数据源上未找到远端数据表。',
        cols: '{{n}} 列',
        cancel: '取消',
        create: '创建 {{n}} 个对象',
      },
      toast: {
        loadFailed: '加载数据源:{{msg}}',
        testOk: '{{name}}:连接正常',
        testOkMs: '{{name}}:连接正常({{ms}}ms)',
        testResult: '{{name}}:{{result}}',
        failed: '失败',
        deleted: '已删除 {{name}}。',
        deleteFailed: '删除 {{name}}:{{msg}}',
        loadDrivers: '加载驱动:{{msg}}',
        openFailed: '打开 {{name}}:{{msg}}',
        updated: '已更新 {{name}}。',
        created: '已创建 {{name}}。',
        saveFailed: '保存:{{msg}}',
        introspectFailed: '读取表结构 {{name}}:{{msg}}',
        synced: '已从 {{name}} 同步 {{n}} 个对象。',
        syncFailed: '失败:{{errors}}',
      },
    },
    cloudConnection: {
      checking: '正在检查连接…',
      ...
      expired: '请求在被批准前已过期,请重新开始。',
      bindingFailed: '绑定失败。',
      waitingPopup: '在刚打开的窗口中批准此连接 —— 本页会自动更新。',
      waitingConsole: '正在等待云控制台批准…',
      openApproval: '打开批准页面',
      copy: '复制',
      copied: '已复制',
      codePrefilled: '验证码已在批准页面预填',
      ifNoWindow: '—— 若窗口未出现,',
      openItHere: '点此打开',
      endPunct: '。',
      cancel: '取消',
      connectedTitle: '已连接到 ObjectStack Cloud',
      fields: {
        runtime: '运行时',
        organization: '组织',
        approvedBy: '批准人',
      ...
      boundNote: '你所在组织的私有软件包现已出现在应用市场的「你的组织」下。',
      disconnect: '断开连接',
      notConnected: '未连接',
      connectDesc:
        '将此运行时连接到 ObjectStack 控制平面,即可浏览并安装你所在组织的私有软件包。批准只需在你的云账户中点一下 —— 无需在本页输入任何 ID 或凭据。',
      connect: '连接',
    },
    loadingSteps: {
      connecting: '正在连接数据源',
