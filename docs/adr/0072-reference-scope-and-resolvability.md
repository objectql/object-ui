...
**Builds on**: [ADR-0032 unified-expression-layer](https://github.com/objectstack-ai/framework/blob/main/docs/adr/0032-unified-expression-layer.md) (one authoring language: CEL), [ADR-0058 expression & predicate surface](https://github.com/objectstack-ai/framework/blob/main/docs/adr/0058-expression-and-predicate-surface.md) (interpret vs compile, fail-policy matrix) — both in the framework repo; [ADR-0036](./0036-field-conditional-rules.md) (field conditional rules, this repo)
**Consumers**: `@object-ui/app-shell` (metadata-admin flow + expression authoring), `@objectstack/spec` (`FlowNode.outputSchema`), `@objectstack/service-automation`, `@objectstack/formula`
**Relates to**: #1934 (flow variable data-picker). The picker/validation already landed incrementally (objectui #1973 / #1975 / #1978 / #1980 / #1981) — this ADR is the *model* those slices are instances of, and the contract for everything past them.

---

## TL;DR

A low-code author writes CEL expressions and `{var}` templates across **dozens of surfaces** (flow/edge conditions, formula fields, validation & visibility predicates, sharing/RLS conditions, screen templates, …). Until #1934 they typed reference names **from memory**, and a slightly-wrong name failed *silently* at runtime (or tripped the `{record.x}` brace-in-CEL trap, ADR-0032). The data-picker (#1973+) fixes discovery for flow; this ADR generalises the **model** behind it.
...
**Other surfaces do not flatten** (research across the framework):

| Surface | Evaluator (file:line) | In-scope references | Token shape |
| :-- | :-- | :-- | :-- |
| Flow / edge / decision | `service-automation/engine.ts:946-964` | `record`,`previous`, **flattened record fields**, flow `variables`, node outputs, `$runId`/`$flowName` | bare **and** `record.x` |
| Formula field (`Field.expression`) | `objectql/engine.ts:119` | `{ now, timezone, user, org, record }` | `record.x` only |
| Validation (`script`/`cross_field`/`when`) | `objectql/.../rule-validator.ts:289` | `{ record: {...previous,...patch}, previous }` | `record.x` only |
| Field `visibleWhen`/`requiredWhen`/`readonlyWhen` | `rule-validator.ts:178-190` | merged `record`, `previous`, (`parent` for master-detail) | `record.x` only |
| Hook lifecycle `condition` | `objectql/hook-wrappers.ts:84` | `{ record }` | `record.x` only |
| RLS `using`/`check` (compile→filter) | `plugin-security/rls-compiler.ts:259` | `current_user.*` (+ pre-resolved membership), record field names | field operands; pushdown subset only |
| Sharing-rule `condition` (compile→filter) | `plugin-sharing/bootstrap-declared-sharing-rules.ts:61` | record fields only | field operands; pushdown subset only |
| Action/view/app `visible` | framework UI layer; roots in `formula/cel-engine.ts:52-62` | `record`,`os`,`user`,`ctx`,`features`,… | `record.x` / namespaced |

The canonical CEL scope roots the engine recognises are declared in `framework/.../formula/cel-engine.ts:52-62`: `record, previous, input, output, os, vars, variables, automation, context, args, item, env, user, step, result, trigger, event, payload, data, params, config, settings, ctx, features, parent`.
...
**`outputSchema` today** (`framework/.../spec/src/automation/flow.zod.ts:129-132`): `Record<string, { type: 'string'|'number'|'boolean'|'object'|'array'; description?: string }>`, **optional**, carries **no `objectName`**, is **not consumed by the engine**, and is read by the designer only for connector/Tool display. So it cannot, today, tell the picker *which object* an `object`-typed output holds. The reliable signal is the **producing node's `config.objectName`** (required on CRUD nodes) found by walking the graph back — exactly the walk the picker already does (`inspectors/flow-scope.ts` `flowAncestors`).

### What already shipped (Slice 0/1, #1934)

The flow data-picker and its inline validator already landed: graph-aware scope resolution (`flow-scope.ts`), the `{x}` picker with bare-vs-`{var}` insertion (`VariableTextInput.tsx`, ADR-0032 brace handling), scope-aware unknown-reference warnings with "did you mean?" (`flow-ref-check.ts`), repeater-cell + Problems-panel surfacing, and `previous.<field>` refs. This ADR frames those as the first instances of the model below — and the contract for extending it.
...
- **Slice 0/1 — shipped (#1973–#1981):** flow scope resolution, picker + brace-correct insertion, scope-aware unknown-ref validation (inline + repeater cells + Problems panel), `previous.<field>`.
