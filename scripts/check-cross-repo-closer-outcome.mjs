...
//
// ## Why this file exists (#5261)
//
// `.github/workflows/cross-repo-issue-closer.yml` carries ~250 lines of inline
// `actions/github-script`, and until this file landed that script was CODE
// NOBODY HAD EVER SEEN RUN. It is a port of the framework's harness, brought
// across with the four hardening rounds it pins; the scenarios and mutations
// below are that file's, adapted to this repository's side of the pair.
//
// The framework's own reason for writing it was that its branch had never had
// a target: zero qualified cross-repo closing keywords over its 1176 most
// recently merged pull requests, measured twice over two independent windows.
// THIS REPOSITORY IS THE OPPOSITE CASE, and the difference is the reason the
// port matters more here than the original did there. Measured 2026-08-19 over
// ALL 3425 merged pull requests in this repository (#1..#5288, the closed-PR
// listing exhausted at 37 pages, the workflow's own regex applied to each
// body):
//
//   - 60 merged pull requests carry a qualified FOREIGN closing keyword -- 61
//     occurrences over 59 distinct targets. 246 more carry the qualified
//     SAME-repo spelling, which the loop skips, and 972 carry the bare `#N`
//     form GitHub handles natively. (Those last two are the counter-probe:
//     they are what makes the 60 a reading rather than a broken regex.)
//   - 15 of the 60 merged AFTER this workflow landed here on 2026-08-09
//     (#3969). In FOURTEEN of those fifteen the framework issue was closed by a
//     seat one to two seconds after the merge -- before the workflow's run had
//     even been created -- and in the fifteenth the close lands in the same
//     second the run is created, with the runner starting later still. So the
//     loop hit `state === 'closed'` fifteen times out of fifteen, skipped each
//     target whole, and left no backlink. None of those fifteen framework
//     issues carries this workflow's backlink comment today, and none of the
//     fifteen pull requests carries its token-absent notice.
//   - all 941 runs of this workflow have reported success.
//
// So the branch this repository maintains for the occasion it exists for has
// had a target roughly 1.5 times a day since it landed, has taken exactly one
// exit every time, and has delivered nothing on every one of them -- greenly.
// Reading is still the only thing that has ever found a defect here; this file
// is what makes reading repeatable.
//
// Two consequences, and this file answers both:
//
//   1. The defects are found by reading, one card at a time -- objectstack#9575
//      for the notice, objectstack#9595 for the loop's verdict,
//      objectstack#9643 for the already-closed target, objectstack#9711 for the
//      pull-request guard -- and each fix lands as more unexercised code.
//   2. Nothing catches a script that does not even COMPILE. github-script
//      compiles the whole block as one `AsyncFunction` body, and the framework
//      copy of this workflow was taken down twice in one day by a `SyntaxError`
//      that no outcome assertion could ever have seen. Assertion 0 below is
//      that compile, on the real text.
//
...
// FAILURE rather than a skip: a workflow that has been renamed, a step that no
// longer uses github-script, an empty `script:` -- all of them exit non-zero
// here, because a harness that could not find its subject has verified nothing
// at all.
//
// ## What is asserted, and what is deliberately not
//
// Asserted: the target parse (which keyword spellings qualify, which forms do
// not, that a same-repo reference is skipped), the target KIND (a pull request
// is also an issue to `issues.get`, so `owner/repo#N` naming a PR reaches the
// loop like anything else -- the scenarios pin that it is REFUSED rather than
// closed, and that the refusal happens before the already-closed branch can
// leave a comment on somebody else's pull request), and the OUTCOME of every
// exit -- which of `core.setFailed` / `core.warning` / a job summary fires, and
// which API calls were made.
//
// Also asserted: that every API the script calls is one this harness MODELS.
// That is not pedantry -- it is a failure the framework copy walked into.
// Adding `listComments` to the shipped script moved real behaviour and all 52
// assertions stayed green, because the unstubbed method threw a `TypeError`
// INSIDE the script's own `try` and was absorbed by the very degradation branch
// the new code had just added. A harness whose stubs lag the script does not
// under-report; it reports a pass about a path the script no longer takes. So
// the guard records the access out-of-band and `judge` fails the scenario on
// the record, whatever that scenario's own assertions say.
//
...
// That is a property of GitHub, and the only honest evidence for it is the 246
// same-repo qualified references measured above, which GitHub did close.
//
...
// was lost, drop the pull-request guard -- and requires the battery to go RED
// for each, naming the scenario it expects. Each mutation also asserts its own
// anchor was PRESENT before substituting: a mutation that silently matched
// nothing would leave the battery green and read exactly like a passing
// self-test.
//
...
const THIS_REPO = { owner: 'objectstack-ai', repo: 'objectui' };
...
function makeDoubles({ body, token, issues = {}, prCommentError = null, summaryError = null }) {
  ...
  const issuesApi = {
    async get({ owner, repo, issue_number: n }) {
      ...
      // `state_reason` is nullable on this endpoint, so the default models
      // that rather than inventing a value the API does not promise.
      //
      ...
      // objectstack-ai/objectstack#9711 (an issue) carries no such key at all.
      // A fixture that always spelled the key would let a truthiness test pass
      // here that the real API would fail.
      return {
        data: {
          state: t.state ?? 'open',
  ...
  // green over a behaviour it never exercised. Measured, on the objectstack#9643 change
  ...
  const context = {
    ...
    payload: {
      pull_request: {
        body,
        html_url: 'https://github.com/objectstack-ai/objectui/pull/1234',
        number: 1234,
      },
    },
  };
...
const MIXED_BODY = [
  'Fixes objectstack-ai/objectstack#456',
  'CLOSES my-org/some.repo#22',
  'resolved third/party#7',
  '',
  'Part of objectstack-ai/objectstack#999 -- a reference, not a close.',
  'Fixes #321 -- bare form, GitHub already handles it.',
  'Fixes objectstack-ai/objectui#5 -- same repo, GitHub already closed it.',
].join('\n');

const FOREIGN = ['objectstack-ai/objectstack#456', 'my-org/some.repo#22', 'third/party#7'];

/** The target the already-closed scenarios (L2, L6-L9) work on. */
const CLOSED_TARGET = 'objectstack-ai/objectstack#456';

/** The target the PULL REQUEST scenarios (L12, L13) work on. */
const PR_TARGET = 'my-org/some.repo#22';

/**
 * The `pull_request` key GitHub returns when an issue number is a pull request.
 *
 * Copied from a real response rather than invented, and re-measured for this
 * copy on 2026-08-19: `GET /repos/objectstack-ai/objectstack/issues/9716` --
 * a merged pull request -- answers exactly these five fields, while
 * `.../issues/9711` -- an issue -- answers with the key ABSENT. The loop
...
const PR_URL = 'https://github.com/objectstack-ai/objectui/pull/1234';
 ...
 */
const MARKER = '<!-- cross-repo-issue-closer:objectstack-ai/objectui#1234 -->';
...
export const SCENARIOS = [
  {
    ...
    scenario: () => ({ body: 'Fixes #12\n\nRefs objectstack-ai/objectstack#9 (not a keyword).', token: 'pat' }),
    check: (r, t) => [
  ...
  },
  {
    ...
    check: (r, t) => {
      ...
      return [
        ...
        t(!line.includes('objectstack-ai/objectui#5'), 'P1 skips the qualified SAME-repo reference'),
        t(!r.calls.get.includes('objectstack-ai/objectui#5'), 'P1 never calls the API for the same-repo reference'),
      ];
    },
  },
  {
    ...
    check: (r, t) => [
      ...
      t(
        ...
        'N1 announces the work order as an annotation too (objectstack#9575)',
      ),
  ...
  {
    id: 'N2',
    name: 'token absent, notice refused -- summary keeps it, job goes red (objectstack#9594)',
  ...
  },
  {
    ...
    check: (r, t) => [
      ...
      t(
        ...
        'L1 comment carries the per-PR marker, so a LATER run can tell this backlink is already there (objectstack#9643)',
      ),
  ...
  {
    id: 'L2',
    name: 'already closed by a human -- the backlink still lands, the close does not (objectstack#9643)',
    scenario: () => ({
      body: MIXED_BODY,
      token: 'pat',
  ...
  {
    id: 'L6',
    name: 'already closed AND already linked (a re-run) -- nothing is posted twice (objectstack#9643)',
    scenario: () => ({
      body: MIXED_BODY,
      token: 'pat',
      issues: {
    ...
    check: (r, t) => [
      ...
      t(
        ...
        'L6 posts NO second backlink -- re-running this job must be idempotent (objectstack#9643 H2)',
      ),
  ...
  {
    id: 'L7',
    name: 'already closed as not_planned -- no backlink, because it would contradict the triage (objectstack#9643)',
    scenario: () => ({
      body: MIXED_BODY,
      token: 'pat',
  ...
  {
    id: 'L8',
    name: 'already closed but the comment listing is refused -- skip AT MOST ONCE, and say so (objectstack#9643)',
    scenario: () => ({
      body: MIXED_BODY,
      token: 'pat',
  ...
  {
    id: 'L9',
    name: 'already closed with a NULL state_reason -- read as completed, so the backlink lands (objectstack#9643)',
    scenario: () => ({
      body: MIXED_BODY,
      token: 'pat',
      // `state_reason` is nullable, so a fix that keys on the reason has to
      // say what null MEANS, and this pins the answer. Modelled rather than
      // sampled on purpose: measured 2026-08-19, every closed ISSUE in this
      // repository's first three pages answers `completed`, and the things
      // that answer `null` are merged PULL REQUESTS
      // (objectstack-ai/objectstack#9716 and objectstack-ai/objectstack#9143).
      // That asymmetry is exactly why the kind guard has to run first -- L13.
  ...
  {
    id: 'L3',
    name: 'ONE target refused -- the rest still close (isolation) AND the job goes red (objectstack#9595)',
    scenario: () => ({
      body: MIXED_BODY,
      token: 'pat',
      issues: { 'objectstack-ai/objectstack#456': { getError: DENIED } },
    }),
    check: (r, t) => [
      ...
      t(
        r.log.warning.some((w) => w.message.includes('objectstack-ai/objectstack#456') && /HTTP 404/.test(w.message)),
      ...
      t((r.log.failed[0] ?? '').includes('objectstack-ai/objectstack#456'), 'L3 setFailed names the target left open'),
      t(
  ...
  {
    id: 'L11',
    name: 'the ROUND TRIP: re-running the job after a clean close posts no second backlink (objectstack#9643 H2)',
    scenario: () => ({ body: MIXED_BODY, token: 'pat' }),
  ...
  {
    id: 'L12',
    name: 'the target is an OPEN pull request -- refused out loud, never closed (objectstack#9711)',
    scenario: () => ({
      body: MIXED_BODY,
      token: 'pat',
    ...
    check: (r, t) => [
      ...
      t(
        ...
        'L12 ANNOUNCES the refusal and names the target -- a quiet `continue` here would have been one '
          + 'more silent exit in a file whose known ones were every one of them found by somebody reading it',
      ),
      t(
  ...
  {
    id: 'L13',
    name: 'the target is a MERGED pull request -- refused BEFORE the already-closed branch comments on it (objectstack#9711)',
    scenario: () => ({
      body: MIXED_BODY,
      token: 'pat',
      // A merged pull request answers `state: 'closed'` with
      // `state_reason: null` from the issues endpoint -- measured on
      // objectstack-ai/objectstack#9143 and #9716. That is the fixture L9 uses to
...
non-zero rather than reporting OK (objectstack#4690). Either ${WORKFLOW}
...
// exercised. That is the same objectstack#4690 shape the extraction guards against, one
// level in.

const MUTATIONS = [
  {
    id: 'M1',
    what: 'the post-loop verdict is downgraded back to a warning (the objectstack#9595 defect, restored)',
    from: "core.setFailed(verdict.join(' '));",
    to: "core.warning(verdict.join(' '));",
    // Since objectstack#9711 the verdict carries both red classes, so this one mutation
  ...
  {
    id: 'M12',
    what: 'the pull-request guard is dropped, so a keyword aimed at a foreign PR comments on it and CLOSES it (objectstack#9711)',
  ...
  {
    id: 'M7',
    what: 'the notice path loses its verdict (the objectstack#9594 half, restored to its defect)',
...
async function selfTest() {
  ...
  // 4. Missing input is a failure, never a pass (objectstack#4690).
  ...
  // 5. Wiring. A check nobody runs is the shape both repositories keep paying
