...
export function analyze(root, /** @type {{ families?: DynamicKeyFamily[] }} */ { families = DYNAMIC_KEY_FAMILIES } = {}) {
  ...
  for (const file of collectSourceFiles(root)) {
    ...
    const walksCallSites = /\btt?\s*(?:\?\.)?\s*\(/.test(text);
    // objectui#7567 — class 8's subject is a table, not a call, and the two do
    // not co-occur: `useTimelineTranslation.ts` and its eight siblings declare a
    // factory and its defaults and never write `t(` at all. Gating the parse on
    // the call-site pre-filter alone would have dropped every one of those
    // tables silently, which is the same shape as the #4117 bug above.
    const declaresFactory = text.includes('createSafeTranslation');
    if (!walksCallSites && !declaresFactory) continue;
