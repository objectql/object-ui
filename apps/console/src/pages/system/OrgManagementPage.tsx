 ...
 * Fetches data via dataSource.find('sys_org') and supports
...
const columns = orgObject.views[0].columns;
...
export function OrgManagementPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const result = await dataSource.find('sys_org');
  ...
  const handleCreate = useCallback(async () => {
    if (!dataSource) return;
    try {
      await dataSource.create('sys_org', { name: 'New Organization', slug: '', plan: 'free', status: 'active' });
  ...
  const handleDelete = useCallback(async (id: string) => {
    if (!dataSource) return;
    try {
      await dataSource.delete('sys_org', id);
